...
import com.example.lostandfound.utils.HttpUtil;
...
public class RegisterActivity extends AppCompatActivity {
    ...
    private void registerUser() {
        String url = "http://172.20.10.6:8080/LostFoundServer/register";
        ...
        HttpUtil.post(url, formBody, new okhttp3.Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                runOnUiThread(() ->
                        Toast.makeText(RegisterActivity.this, "网络请求失败，请检查连接", Toast.LENGTH_SHORT).show()
                );
            }
                ...
                runOnUiThread(() -> {
                    try {
                        ...
                        if ("success".equals(status)) {
                            ...
                            if (message.contains("用户名已被注册")) {
                                tilUsername.setError(message);
                            } else if (message.contains("手机号已被注册")) {
                                tilPhone.setError(message);
                            } else {
                                Toast.makeText(RegisterActivity.this, "注册失败: " + message, Toast.LENGTH_LONG).show();
                            }
                        }
                    } catch (Exception e) {
                        Toast.makeText(RegisterActivity.this, "返回数据解析失败", Toast.LENGTH_SHORT).show();
                        e.printStackTrace();
                    }
                });
            }
